...
int main(int argc, char* argv[]) {
  ...
  if(argc < defaultLength) { 
    printf("Error: <Nmodes> <Nquad> <1-Roothaan 2-Diis 3-GradDescent> <Diis_subspace> <Mode Selection> <EnergyFile> <DipoleXFile> <DipoleYFile> <DipoleZFile> <CouplingDegree> [<EnergyFile> <Dx> <Dy> <Dz> <dim> ...]\n");
    exit(0);
  }
  ...
  int modeSelection = atoi(argv[5]);
  std::vector<std::string> potFileNames;
  potFileNames.push_back(argv[6]); //arg4
  std::vector<std::string> dipFileNames;
  dipFileNames.push_back(argv[7]); //arg5
  dipFileNames.push_back(argv[8]); //arg6
  dipFileNames.push_back(argv[9]); //arg7
  std::vector<int> potDims;
  potDims.push_back(atoi(argv[10])); //arg8
...
/////////////////Create Mode, EigSolver, Potential Objects///////////////////
  ...
  char filename[1024];
  if(conv==1) {
    // std::string name = "eemVSCF_roothaanAsDIIS.dat";
    // strcpy(filename,name.c_str());
     sprintf(filename,"eemVSCF_roothaan%i.dat",modeSelection);     
  } else if(conv==2) {
    sprintf(filename,"eemVSCF_diis%i_%i.dat",diis_subspace,modeSelection);     
  } else
    std::string name = "eemVSCF_gradDescent.dat";
  FILE *results = fopen(filename,"w");
//===============================Begin VSCF==================================
//for(int banana=0 ; banana<25 ; banana++) {
//if(modeSelection == nModes) {
  printf("==========GS\n");
