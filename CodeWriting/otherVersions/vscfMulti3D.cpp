...
//================Get info from user if more than one method is used====================
    potDims.push_back(2);
    potDims.push_back(3);
    potFileNames.push_back("V2.dat");
    potFileNames.push_back("V3.dat");
/*    if(numMethods > 1) {
...
std::time_t finish = std::chrono::system_clock::to_time_t(end);
std::cout << "Set-up done: " << std::ctime(&finish);
...
finish = std::chrono::system_clock::to_time_t(end);
std::cout << "Ground state done: " << std::ctime(&finish) << '\n';
...
finish = std::chrono::system_clock::to_time_t(end);
std::cout << "Ground state failed: " << std::ctime(&finish) << '\n';
...
finish = std::chrono::system_clock::to_time_t(end);
std::cout << "Excited state " << z << " done: " << std::ctime(&finish) << '\n';
...
finish = std::chrono::system_clock::to_time_t(end);
std::cout << "Excited state " << z << " failed: " << std::ctime(&finish) << '\n';
}}
}
