...
namespace
{
  inline void rot(double* a, const double s, const double tau, const int i, const int j, const int k, const int l, const int n)
  {
    double g,h;
    g=a[i*n+j];
    h=a[k*n+l];
...
void jacobi(double* a, double* d, double* v, int n)
{
  ...
  for(int i=1 ; i<50 ; i++)
  {
    ...
    if(sm==0.0) { 
      free(b);
      free(z);
      return;               //normal return, relies on quad'c convgence to machine underflow
    }
    if(i<4)
